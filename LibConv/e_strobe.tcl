...
namespace eval SptParser {
    ...
    snit::type EStrobe {
        ...
        typevariable Cmd -array { open "open_shutter" close "close_shutter" strobe "strobe" }
    	...
    	typemethod parse { ct fix i data } {
    		...
    		for {incr i} { $i<[llength $data] } {incr i } {
				...
				if { [SptParser::parse_channeltype $line] } { 
					SptParser::PutsVars i line
				} elseif { [regexp -nocase {(open)([ \t]+)(from)([ \t]+)([0-9]+)([ \t]+)(to)([ \t]+)([0-9]+)} $line p key1 s1 key2 s2 from s3 key3 s4 to] } {
					lappend fixture(CHANNELS) $channel
					lappend fixture($channel,COMMAND) $Cmd(open)
					lappend fixture($channel,$Cmd(open)) [list $from $to ONOFF]
